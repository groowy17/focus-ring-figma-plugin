...
// Variables
const COMPONENT_NAME: string = "fosus-ring";
...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'SET_FOCUS_RING') {
    ...
    const focusRingComponent = figma.currentPage.findOne(node => node.name === COMPONENT_NAME) as ComponentNode;

    nodes.forEach((item: SceneNode) => {
      ...
      if (item.type == "RECTANGLE" || item.type == "ELLIPSE") {
        ...
        if (focusRingComponent) { // if component on current page
          ...
          let groupWithRing: GroupNode = figma.group([item, focusRing], item.parent);
...
const getFocusRingComponent = (width: number, height: number): ComponentNode => {
  const focusRingComponent: ComponentNode = figma.createComponent();
  const rectOutlineNeutral: RectangleNode = figma.createRectangle(); // white
  const rectOutlineAccent: RectangleNode = figma.createRectangle(); // blue
  const constr: Constraints = {horizontal: "STRETCH", vertical: "STRETCH"};
  ...
  const groupRect: GroupNode = figma.group([rectOutlineNeutral, rectOutlineAccent], focusRingComponent);
